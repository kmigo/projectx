...
class TakePhotoFrontDocument extends StatelessWidget {

  final Function(dynamic) onSuccess;
  final String typeDocument;

  const TakePhotoFrontDocument({super.key,required this.typeDocument ,required this.onSuccess,});
  ...
  Widget build(BuildContext context) {
    return  Column(
          ...
          children: [
            ...
            const UolletiText.labelXLarge('Frente do documento',bold: true,),
            const SizedBox(height: 16,),
           ...
           onPressed : () async{
            ...
            if(photo != null && photo.runtimeType == String){
              final decodedBytes = base64Decode((photo as String).split(',').last);

              final filename = '${typeDocument}front.jpg'; 

              onSuccess(CustomMultipartFile.generateFile(filename, decodedBytes));
            }
           },
            ),
            const SizedBox(height: 16,),
